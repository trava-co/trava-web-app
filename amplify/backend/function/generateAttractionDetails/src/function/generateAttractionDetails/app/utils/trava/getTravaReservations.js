...
exports.getTravaReservations = void 0;
...
const getTravaReservations = async ({ attractionId, attractionName, destinationName, relevanceMap, attractionType, reservable, // google maps attribute. if falsy, then we know it's not reservable. else, it's reservable.
planning, }) => {
    ...
    if (isTypeEat && !reservable) {
        ...
        return API_1.ATTRACTION_RESERVATION.NOT_TAKEN;
    }
    // create deep copy of reservations so that deletions don't affect the original
    const possibleReservationValues = JSON.parse(JSON.stringify(reservations_1.reservationsMap));
    ...
    const planningItemWithReservationsRequired = planning?.find((item) => item.name.toLowerCase().includes('reservations required'));
    if (planningItemWithReservationsRequired) {
        return API_1.ATTRACTION_RESERVATION.REQUIRED;
    }
    ...
    const planningItemWithReservationsRecommended = planning?.find((item) => item.name.toLowerCase().includes('reservations recommended'));
    if (planningItemWithReservationsRecommended) {
        return API_1.ATTRACTION_RESERVATION.RECOMMENDED;
    }
    // else, fallback to using openai
    if (isTypeEat) {
        // reservable is true, so we know that reservations are taken
        delete possibleReservationValues[reservations_1.DESCRIPTIVE_RESERVATION_NAMES.RESERVATIONS_NOT_TAKEN];
    }
    const reservationQuestion = questions_1.questions[attractionType].RESERVATIONS;
    ...
    const buildChatThread = (inputWithNewSentence) => {
        return (0, reservations_1.getTravaReservationsInfoChatThread)({
            attractionName,
            destinationName,
            attractionType,
        ...
        functions = [
            {
                ...
                parameters: {
                    ...
                    properties: {
                        reservation: {
                            type: 'string',
                            description: 'Whether this attraction requires, recommends, optionally takes, or does not take reservations',
        ...
        functions = [
            {
                ...
                parameters: {
                    ...
                    properties: {
                        reservation: {
                            type: 'string',
                            description: 'Whether this restaurant requires, recommends, optionally takes, or does not take reservations',
                            enum: Object.keys(possibleReservationValues),
                        },
                    },
                    required: ['reservation'],
    ...
    let openAIReservationsInput;
    try {
        openAIReservationsInput = (0, buildRelevantInputText_1.buildRelevantInputText)({
    ...
    console.log(`chat completions for ${attractionName} in ${destinationName} responded with reservation status: ${reservation}`);
    if (possibleReservationValues[reservation]) {
        return possibleReservationValues[reservation];
    }
    (0, logError_1.logError)({
        error: new Error(`getTravaReservations GPT-3 returned an improper value: ${reservation}. Falling back to GPT-4. Attraction name: ${attractionName}. Destination name: ${destinationName}.`),
    });
    ...
    const possiblyValidValue = await (0, mostSimilarResponse_1.getMostSimilarResponse)({
        context: mostSimilarResponse_1.DESCRIPTIVE_CONTEXT.RESERVATION,
        offSchemaValue: reservation,
        possibleValues: Object.keys(possibleReservationValues),
    });
    console.log(`gpt-4 chat completions for ${attractionName} in ${destinationName} responded with reservation status: ${possiblyValidValue} as being most similar to ${reservation}`);
    if (possibleReservationValues[possiblyValidValue]) {
        return possibleReservationValues[possiblyValidValue];
    }
    (0, logError_1.logError)({
        error: new Error(`getTravaReservations GPT-4 returned an improper value: ${possiblyValidValue}. Falling back to cosine similarity.`),
    });
    ...
    let nonTravaReservationEmbeddings;
    try {
        nonTravaReservationEmbeddings = await (0, createEmbeddings_1.createEmbeddings)({
            input: [reservation],
            attractionId,
            attractionName,
            destinationName,
    ...
    for (let key of Object.keys(possibleReservationValues)) {
        const similarity = (0, cosineSimilarities_1.cosineSimilarity)(nonTravaReservationEmbedding, embeddings_1.reservationEmbeddings[key]);
        similarities[key] = similarity;
        console.log(`Cosine similarity for ${key}: ${similarity}`);
    }
    const travaReservationDescriptiveName = Object.keys(similarities).reduce((a, b) => similarities[a] > similarities[b] ? a : b);
    const travaReservation = reservations_1.reservationsMap[travaReservationDescriptiveName];
    console.log(`closest reservation value to ${reservation} is ${travaReservation}`);
    return travaReservation;
};
exports.getTravaReservations = getTravaReservations;
