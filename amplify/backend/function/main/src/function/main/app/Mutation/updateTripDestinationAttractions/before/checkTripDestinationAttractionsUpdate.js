...
const getTripDestinationAttraction_1 = __importDefault(require("../../../utils/getTripDestinationAttraction"));
const checkTripDestinationAttractionsUpdate = (event) => __awaiter(void 0, void 0, void 0, function* () {
    if (!(event.identity && 'sub' in event.identity)) {
        throw new Error(lambdaErrors_1.CUSTOM_NOT_AUTHORIZED_UPDATE_TRIP_DESTINATION_ATTRACTION_MESSAGE);
    }
    ...
    // check if attractions already exist within the voting deck (they must in order to update)
    for (const attraction of event.arguments.input.attractions) {
        const alreadyInTrip = yield (0, getTripDestinationAttraction_1.default)({
...
exports.default = checkTripDestinationAttractionsUpdate;
