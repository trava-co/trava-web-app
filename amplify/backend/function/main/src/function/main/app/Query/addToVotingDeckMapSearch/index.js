...
const createGetAttractionOpenSearchQuery = ({ attractionId, centerCoords, destinationDates, userId, bucketListedAttractionIds, deckAttractionIds, }) => {
    const query = {
        bool: {
            filter: {
                term: {
...
const createSearchAttractionsOpenSearchQuery = ({ searchStringEmbedding, attractionType, attractionCategories, attractionCuisine, boundingBox, centerCoords, destinationDates, userId, bucketListedAttractionIds, deckAttractionIds, }) => {
...
const addToVotingDeckMapSearch = (event) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s, _t, _u;
    console.log('addToVotingDeckMapSearch');
    ...
    const { tripId, destinationId, searchString, attractionType, attractionCategories, attractionCuisine, boundingBox, centerCoords, destinationDates, selectedAttractionId, } = event.arguments.input;
    ...
    let votingDeckMapOpenSearchQuery;
    if (selectedAttractionId) {
        // get this specific attraction
        votingDeckMapOpenSearchQuery = createGetAttractionOpenSearchQuery({
        ...
        votingDeckMapOpenSearchQuery = createSearchAttractionsOpenSearchQuery({
    ...
    const msearchQueryParts = [
        (0, createQueryObjects_1.createQueryObjects)(votingDeckMapOpenSearchQuery),
    ...
    // @ts-ignore
    const [votingDeckMapOpenSearchQueryResponse, statsResponse] = msearchResponse.responses || [];
    const numberOfNearbyAttractions = votingDeckMapOpenSearchQueryResponse.hits.total.value;
    ...
    // @ts-ignore
    const attractions = votingDeckMapOpenSearchQueryResponse.hits.hits
        .filter((item, index) => !!selectedAttractionId || !relevanceThreshold || item._score >= relevanceThreshold)
        .map(getAddToVotingDeckSearchItem);
    return {
        __typename: 'AddToVotingDeckMapSearchResponse',
        attractions,
    };
});
exports.default = addToVotingDeckMapSearch;
