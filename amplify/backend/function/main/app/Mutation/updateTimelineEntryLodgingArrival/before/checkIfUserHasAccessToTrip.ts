...
import { CUSTOM_NOT_AUTHORIZED_UPDATE_TIMELINE_ENTRY_LODGING_ARRIVAL_MESSAGE } from 'shared-types/lambdaErrors'
...
> = async (event) => {
  ...
  if (!(event.identity && 'sub' in event.identity)) {
    throw new Error(CUSTOM_NOT_AUTHORIZED_UPDATE_TIMELINE_ENTRY_LODGING_ARRIVAL_MESSAGE)
  }
