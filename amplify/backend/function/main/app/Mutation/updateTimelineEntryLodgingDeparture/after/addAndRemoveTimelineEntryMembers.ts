...
const addAndRemoveTimelineEntryMembers = async (
  event: AppSyncResolverEvent<UpdateTimelineEntryLodgingDepartureMutationVariables>,
  timelineEntryLodgingDeparture: TimelineEntry,
) => {
  ...
  const timelineEntry = await getTimelineEntry(timelineEntryLodgingDeparture.id)
