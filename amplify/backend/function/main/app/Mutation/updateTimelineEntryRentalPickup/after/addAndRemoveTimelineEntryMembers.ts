...
const addAndRemoveTimelineEntryMembers = async (
  event: AppSyncResolverEvent<UpdateTimelineEntryRentalPickupMutationVariables>,
  timelineEntryRentalPickup: TimelineEntry,
) => {
  ...
  const timelineEntry = await getTimelineEntry(timelineEntryRentalPickup.id)
