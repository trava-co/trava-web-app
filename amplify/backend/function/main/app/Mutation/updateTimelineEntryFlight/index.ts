...
async function _privateUpdateTimelineEntryFlight(variables: UpdateTimelineEntryFlightMutationVariables) {
...
const createTimelineEntryFlight: AppSyncResolverHandler<UpdateTimelineEntryFlightMutationVariables, any> = async (
  event,
  ...args
) => {
  /**
  ...
  const timelineEntryFlight = await _privateUpdateTimelineEntryFlight(event.arguments)

  if (!timelineEntryFlight) {
    throw new Error('Failed to update timeline entry flight')
  }
  ...
  for (const hook of syncAfterHooks) {
    ...
    await hook(event, timelineEntryFlight)
  }
