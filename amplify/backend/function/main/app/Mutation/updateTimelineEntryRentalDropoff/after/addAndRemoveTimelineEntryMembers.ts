...
const addAndRemoveTimelineEntryMembers = async (
  event: AppSyncResolverEvent<UpdateTimelineEntryRentalDropoffMutationVariables>,
  timelineEntryRentalDropoff: TimelineEntry,
) => {
  ...
  const timelineEntry = await getTimelineEntry(timelineEntryRentalDropoff.id)
