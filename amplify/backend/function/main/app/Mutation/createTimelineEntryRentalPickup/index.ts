...
async function _privateCreateTimelineEntryRentalPickup(variables: CreateTimelineEntryRentalPickupMutationVariables) {
  ...
  const res = await ApiClient.get()
    ...
    .apiFetch<PrivateCreateTimelineEntryMutationVariables, PrivateCreateTimelineEntryMutation>({
      ...
      variables: {
        input: {
          timelineEntryType: TimelineEntryType.RENTAL_PICKUP,
...
> = async (event, ...args) => {
  ...
  const timelineEntryRentalPickup = await _privateCreateTimelineEntryRentalPickup(event.arguments)

  if (!timelineEntryRentalPickup) {
    throw new Error('Failed to create timeline entry rental pickup')
  }

  for (const userId of event.arguments.input?.memberIds) {
    const timelineEntryMembers = await _privateCreateTimelineEntryMember({
      input: {
        timelineEntryId: timelineEntryRentalPickup.id,
        userId: userId,
      },
    })
